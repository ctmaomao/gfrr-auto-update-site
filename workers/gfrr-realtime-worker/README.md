...
- **Preview 管道（v28.0B，free-tier safe）**：
  ...
  - v28.0G-9B Frontend Asset Version Bump Helper 属于前端静态资源维护工具，不改变 Worker runtime。命令为 `node scripts/bump-frontend-asset-version.mjs odp-global-stock-gap-1` 或 `npm run bump:frontend-asset-version -- odp-global-stock-gap-1`；当前正式 frontend asset cache version 以 `scripts/app.js` 的 `APP_VERSION` 为准（现 `odp-global-stock-gap-1`）。工具不访问网络、不写 KV、不写 data/realtime、不 deploy Worker。Worker runtime 改动不需要 bump frontend asset version，除非同时修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js`。frontend asset cache version must be bumped when index.html or frontend JS changes。
