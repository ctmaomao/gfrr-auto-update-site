...
- **Preview 管道（v28.0B，free-tier safe）**：
  ...
  - v28.0E-3 在独立 secondary preview 链路新增 **US10Y via Yahoo `^TNX`** 诊断。US10Y secondary 只写入 `/market.secondary-preview.json` / `market:secondary-preview`，不写入主 Worker preview，不覆盖 `values.us10y`，不参与 scoring / decision；Yahoo `^TNX` raw value 按 `divide-by-10` 归一化后作为 diagnostic `value`，并保留 `rawValue`。US10Y 失败不得影响 VIX / Gold / DXY secondary 或主 Worker preview。当前 secondary diagnostics 包含 VIX via Cboe、Gold via Yahoo `GC=F`、DXY via Yahoo `DX-Y.NYB` 与 US10Y via Yahoo `^TNX`。
