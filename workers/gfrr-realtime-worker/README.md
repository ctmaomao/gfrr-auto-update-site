...
- **Preview 管道（v28.0B，free-tier safe）**：
  ...
  - v28.0D-8B-lite 增加低频隔离的 `brentValidation.sourceProbe`：每 **60** 分钟最多运行一次，60 分钟内复用上一轮 main preview 中的 probe 摘要。当前只探测 Google Finance canonical / front-month 两个 URL(**Stooq `brn.f` / `brn.c` / `bz.f` 三个 symbol 探针已于 F6 删除,见下方 v28.0F6**)。sourceProbe 不保存完整 HTML / CSV，不参与 `brentValidation.consensus`、`brentValidation.promotion` 或 `values.brent`；当前可靠 Brent 主逻辑仍是 FRED anchor + Yahoo `BZ=F` / Trading Economics confirmed promotion。**v28.0D-8B findings（历史观测;当时 Google 2 路 + Stooq 3 路 probe 并存）**：当时线上典型快照显示这五路 probe 均未给出可靠 Brent close（Google Finance 两处 `unreliable-html-parse`；Stooq `brn.f`/`bz.f` `empty-body`；`brn.c` `header-unrecognized`），因此当时判定 Google / Stooq probe 均不得升级为 validation source。**Stooq probe 已于 F6 删除,当前 worker 诊断仅剩 2 路 Google Finance probe**;即使某个 probe 返回 `ok`，也只代表后续可另开 **D-8C** 讨论是否升级为 validation source。
  ...
  - v28.0G-9B Frontend Asset Version Bump Helper 属于前端静态资源维护工具，不改变 Worker runtime。命令为 `node scripts/bump-frontend-asset-version.mjs odp-thermal-facility-window-quality-1` 或 `npm run bump:frontend-asset-version -- odp-thermal-facility-window-quality-1`；当前正式 frontend asset cache version 以 `scripts/app.js` 的 `APP_VERSION` 为准（现 `odp-thermal-facility-window-quality-1`）。工具不访问网络、不写 KV、不写 data/realtime、不 deploy Worker。Worker runtime 改动不需要 bump frontend asset version，除非同时修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js`。frontend asset cache version must be bumped when index.html or frontend JS changes。
