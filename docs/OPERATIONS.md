...
**v28.0G-4A Trading Economics observedAt audit**：Worker generated preview 会尝试从 Trading Economics Brent 页面解析 `observedAt`，并在 `brentValidation.promotion.confirmationSources` 与 `brentValidation.audit.candidateSources` 显示 `observedAt` / `ageHours` / `freshnessStatus` / `freshnessReason`。这是 audit-only：解析失败会显示 `freshnessStatus: unknown` 与 `tradingeconomics-observedAt-unparsed`，但不会阻止 Brent promotion，不改变 `values.brent`、Yahoo freshness gate、Yahoo/Trading Economics divergence gate 或 D-6 extreme-move guard。旧 Draft PR #53 不应直接 merge；若 G-4A 观察到 Trading Economics `observedAt` 稳定，再另开 G-4B 讨论是否作为 hard gate。

**v28.0G-4B decision: Trading Economics freshness hard gate**：G-4B 是 decision review，不是 runtime change；G-4B does not change runtime behavior，当前线上仍是 G-4A audit-only。基于 G-4A overnight observation 与 Worker Health scheduled 稳定，建议进入 **G-4C** 实现保守 hard gate：Trading Economics 必须 `ok === true`、value 为正 finite number、`observedAt` 可解析、`ageHours` finite，且 `ageHours <= BRENT_CONFIRMATION_FRESH_HOURS`（48 小时）。若 `observedAt` 不可解析，G-4C 应阻止 promotion 并使用 `tradingeconomics-observedAt-invalid`，同时继续在 `confirmationSources` 显示 `freshnessStatus: unknown` / `tradingeconomics-observedAt-unparsed`；若超过 48 小时，G-4C 应阻止 promotion 并使用 `tradingeconomics-confirmation-stale`，显示 `freshnessStatus: stale` / `tradingeconomics-observedAt-stale`。D-6 `confirmed-extreme-move` 也应要求 TE freshness fresh；Yahoo fresh 但 TE stale / unknown 时不应确认 extreme move。旧 PR #53 已 superseded，不应 merge；G-4C 必须基于最新 main 串行实现，并在部署后做 live validation 与 1-2 轮 scheduled Check Worker Health observation。
