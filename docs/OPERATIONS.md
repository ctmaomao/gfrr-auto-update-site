...
1. 先看页面 frontend version 是否为当前版本（以 `scripts/app.js` 的 `APP_VERSION` 为准，现 `frontend-failclosed-fallback-1`）。
...
1. 检查 live frontend version 是否为当前版本（以 `scripts/app.js` 的 `APP_VERSION` 为准，现 `frontend-failclosed-fallback-1`）。
...
```text
index.html app.js entry → ?v=frontend-failclosed-fallback-1
scripts/app.js and active scripts/modules/*.js local imports → ?v=frontend-failclosed-fallback-1
...
该工具用于以后前端 HTML / JS 改动时统一 bump cache version。当前正式版本仍是 `frontend-failclosed-fallback-1`，不要在没有前端发布需要时最终留下测试版本。工具不访问网络、不写 KV、不写 `data/*.json` / `realtime/*.json`、不 deploy Worker。
