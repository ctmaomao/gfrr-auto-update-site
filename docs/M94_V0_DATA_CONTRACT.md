...
>
> **CURRENT IA OVERRIDE (2026-08-11):** ADR-0022 replaces the former fifth appendix `#external-ai-auxiliary` with an integrated `#macro-risk-editorial` article immediately after the deterministic Hero inside `#macro-risk-overview`. The appendix count is now four; jump-nav remains 16 items with item 15 pointing to `#macro-risk-editorial`. Any in-body “5 appendix / external-ai-auxiliary” wording below is historical M-94 implementation residue. Current UI authority is `DESIGN.md` + ADR-0022.
>
> **NARRATIVE-FIRST IA OVERRIDE (2026-08-11):** ADR-0023 supersedes only the runtime placement/order described in the previous paragraph and §8. `#wow-key-changes` now follows the integrated editorial; Pressure Sources, Signal Layers, Risk Engines, Cross Validation, and Macro Coherence retain their component/data contracts but live inside local `#macro-professional-evidence`. The fold is closed when the AI editorial is valid and automatically opens when it is ineligible. Jump-nav is consolidated to 13 items. Current UI authority is `DESIGN.md` v2.1 + ADR-0023; the detailed component contracts below remain valid.
