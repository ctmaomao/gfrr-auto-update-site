...
- 综合评分 **7.6 / 10**（评分口径与权重见「评分明细」；本分数为加权值 7.625 的取整）。
...
| 11 | `check:*` 可达性 | 见 [分类提案](CHECK_ALL_COVERAGE_CLASSIFICATION.md) §1.1 的权威展开算法 | **227 / 244 可达，17 项不可达**（2026-09-19 新增 `check:odp-news-source-health` 后为 228 / 245；2026-09-21 新增 `check:frontend-asset-version` 后为 229 / 246） |
...
| **定稿** | **227 / 244 可达** | 直接复用 `tests/unit/check-suite-wiring.test.mjs` 的权威展开算法（入口自身计入）。2026-09-19 新增 `check:odp-news-source-health` 后为 228 / 245；2026-09-21 新增 `check:frontend-asset-version` 后为 229 / 246 |
