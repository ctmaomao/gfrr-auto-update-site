...
| npm 脚本总数 | 415 |
| `check:*` 脚本总数 | 245 |
| 从 `check:all` 可达的 `check:*` | **228** |
| 未可达的 `check:*` | **17** |

> 228 + 17 = 245。另有入口 `check:all` 本身计入可达集合（其命令内不含对自身的引用）。
>
> 计数会随 `check:all` 组成变化而变：2026-09-19 新增 `check:odp-news-source-health` 后由 227 / 244 变为 228 / 245。**未可达数保持 17**，因为新检查已纳入 `oil-directional` 套件。
