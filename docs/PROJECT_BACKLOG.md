...
| 项 | 值 |
|---|---|
| 当前生产状态 | v28.0M-73 (null-zero display guard fix; no data/workflow/Worker/scoring change) |
| Cache version | `28.0M-73V` |
| check:all 项数 | 74 |
| 最后审计日期 | **2026-05-21** (M-73 null-zero display guard fix; M-72 macro-driver date display fix; M-71 Brent public proxy source review; M-70 CRE FRED commercialRealEstate ingestion; M-69 Chicago Fed CARTS consumerRetail ingestion; M-68 employment breadth; M-67 ISM PMI source repair; M-63c ACLED reminder workflows; M-66 legacy anchor + subsection kicker polish; ADR-0014 IA contract authority hierarchy; M-63b ACLED monthly ingestion) |
...
| Null-zero display guard | M-73 起缺失 BGCR/TGCR repo spread 和 ICE Brent diagnostic null 值不得显示或分类为 `+0.0bp` / `0.00`；`check:null-zero-display-guards` 已接入 `check:all` |
...
| M-72 | Macro driver date display fix (P2-10) | `e9727dd` | 2026-05-21 | ✅ Fixes JOLTS month and CRE quarter rendering so ISO vintage strings no longer become `undefined` / `NaN` / `QNaN`; cache bumped to 28.0M-72V |
| M-73 | Null-zero display guard fix | (this PR) | 2026-05-21 | ✅ Missing BGCR/TGCR repo spread and ICE Brent diagnostic `null` values no longer render/classify as `+0.0bp` / `0.00`; adds `check:null-zero-display-guards`; `check:all` 73 → 74; cache bumped to 28.0M-73V; no data/workflow/Worker/scoring/decision/execution/position change |
...
| 2026-05-21 | M-72 macro-driver date display fix | Codex | P2-10 closed | JOLTS / CRE vintage formatter no longer produces `undefined` / `NaN` / `QNaN`; cache 28.0M-72V |
| 2026-05-21 | M-73 null-zero display guard fix | Codex | P2 display bugfix | Missing repo spread and ICE Brent diagnostic null values remain missing instead of showing zero-like values; `check:all` 73 → 74; frontend cache 28.0M-73V |
...
### Session Handoff (2026-05-21 — M-73 null-zero display guard ready)

- **上次会话结束于**: HEAD 待 PR。本 session (2026-05-21) 增量：(1) **M-73 null-zero display guard fix**：`buildCrossValidationMatrix.js` 的 `finite()` 不再把 `null` / `undefined` / empty string 视为 0，缺失 BGCR/TGCR repo spread 现在保留为 `missingEvidence`，不再生成 `repo_zero_stress +0.0bp` 或 `repo_stress +0.0bp`；(2) `render.js` 的 Brent value formatter 不再把 ICE missing/null 诊断源显示成 `0.00`，改为缺失值 `--`；(3) 新增 `check:null-zero-display-guards` 并接入 `check:all`，`check:all` = **74 项**；(4) frontend cache bump `28.0M-72V` → `28.0M-73V`。本 PR 符合 DESIGN.md 的所有规则；未改 data/realtime/workflow/Worker/scoring/decision/execution/position。
- **当前进行中**: M-73 小修复 PR 待 review / merge。
- **下一步建议**: (a) PR merge 后等 Pages deploy 完成，线上抽查 Brent 验证源 ICE 显示 `-- / 缺失`，cross-validation 不再出现 `repo_zero_stress +0.0bp`；(b) 另开独立任务处理 external AI stale panel（当前 externalAiInterpretationLayer 的 sourceDataUpdatedAt 落后一轮 Daily）；(c) 若继续 Brent source path,下一 rung 应使用下一个空 M 编号做 proof-of-source design，仍保持 no live fetch / no production data write；(d) M-63c 跨周/monthly cron 仍按原时间点观察。
- **阻塞或等待**: 无技术阻塞。M-63c 跨周 idempotency 待 2026-05-26 Tuesday cron 自然 fire 验证；monthly cron 待 2026-06-09 fire 验证；M-73 仅修 display/classification null guard，不处理 external AI stale 内容刷新。
