...
| 项 | 当前值 |
|---|---|
| 当前生产状态 | v28.0N-1 editorial first-fold + Stage 6A China 10Y/CFETS + Stage 6C China CPI/PPI/PMI + Stage V2X 欧元区波动率(VSTOXX)live display-only 卡;Stage 7 C5 World Order 暂代占位卡退场(C5 = 4 张 live);Stage 8 小批收尾;Stage 9 C6 intro 三分类勘误;Stage 10-13 P3-16 China Macro Liquidity/Property 层全 live(70 城房价 + OMO + 社融 + MLF;C6 = 11 张 live)|
| Cache version | `stage-13-pboc-mlf-1` |
...
- 状态: **70城/OMO/社融/MLF 四源全 live(`chinaPropertyPrice`/`chinaOmo`/`chinaTsf`/`chinaMlf`,Stage 10-13),audit-only**;SLO 仅历史/inactive。详见 [`CHINA_MACRO_LIQUIDITY_PROPERTY_SOURCE_REVIEW.md`](CHINA_MACRO_LIQUIDITY_PROPERTY_SOURCE_REVIEW.md)。
- unlock: **P3-16 实施源全部完成(Stage 10-13)**;SLO 无近期常态操作不追;未来若要更细分项/更高频可另开 stage。
...
- **上次会话结束于**: P3-16 China Macro Liquidity/Property 四源全部代码合并(Stage 10 70城 `d15f3da`/`e03c36a`/`3433326`、Stage 11 OMO `53ca93a`、Stage 12 社融 `eb0c47e`、Stage 13 MLF `9116bb0` + 各 backlog 收尾,HEAD 含 `93af042`)。**线上 Daily run 验证后发现关键问题(见下)。** 会话累计:6-脆弱批、Stage 7-13、UX 清理、hotfix、China-macro source-review、70城明细折叠等全收口并 push。
- **当前进行中**: ⚠️ **关键发现:`pbc.gov.cn` 在 GitHub US runner 上被地理封锁** —— Daily run(updatedAt 2026-05-30T06:04)实测:`chinaOmo`/`chinaTsf`/`chinaMlf`(全 pbc.gov.cn 源)= **`missing`**;而 stats.gov.cn(70城/CPI/PMI)、chinabond(10Y)、chinamoney(CFETS)、boerse(V2X)**全 live**。即域名级封锁(央行站挡境外 IP),非一次性故障。**fail-closed 生效**(三卡显示 OBS·缺失,无伪造数据,边界守住),但 OMO/社融/MLF 三卡在生产环境无数据。
- **下一步建议**: **换源方案(已与 owner 议定方向)** —— 不抓 pbc.gov.cn、不用代理,改抓**境外可达的商业聚合源**(转载同样央行数据):社融→AKShare `macro_china_shrzgm` 背后的 `data.mofcom.gov.cn`(商务部,非 pbc)或东方财富 `data.eastmoney.com/cjsj/` JSON;OMO/MLF→东方财富数据中心 / Trading Economics。**AKShare(`akfamily/akshare`)已把这些端点映射好**(我们用不了 Python 库但照搬 URL)。下一步 = 开「**pbc.gov.cn → eastmoney/mofcom 聚合源切换**」调研:Codex 实证各聚合站确切 JSON 端点 + **从真实 GitHub runner 试抓可达性**(eastmoney/mofcom 商业/部委站大概率可达但未验)。**纪律**:聚合源 source/notes 必须标「东方财富聚合 / TE 公开」,**不冒充 PBOC 官方**(同 Yahoo/TE 代理边界);仍 audit-only。
- **阻塞或等待**: 等 owner 拍板是否开「换 eastmoney 聚合源」调研(我已查到候选:AKShare 文档 `macro_china_shrzgm`→mofcom、`macro_china_lpr`→eastmoney;东财数据中心 `data.eastmoney.com/cjsj/`+`datacenter-web.eastmoney.com` JSON;TE `china/reverse-repo-rate`)。70城/OMO/社融/MLF 的 outline+brief 在 `.claude/stage-briefs/`(gitignored,STAGE_10~13)。代码无 bug,纯属 pbc 源境外被挡。
