...
- **当前进行中**: Stage 13 PBOC MLF 已上线(commit `9116bb0`,`macroDrivers.chinaMlf`,C6 10→11 live)+ backlog 收尾(本次)。**至此 P3-16 China Macro Liquidity/Property 四源(70城/OMO/社融/MLF)全部 live 收官。** 前序 Stage 10-12 + 70城明细 + UX + hotfix + acled 刷新均收口。**Stage 13 待线上 Daily run 验证**(chinaMlf live、操作量/期限合理、利率 null 或合理;OMO/社融 等也一并看)。
- **下一步建议**: P3-16 实施完成。剩 SLO 仅历史(不追)。后续为全新方向待 owner 拍板(候选:① 各源线上验证一轮;② C6 卡序/视觉调整〔现 11 张〕;③ 小缺口 ISM tone〔已修〕之外的产品方向;④ FRED OECD 陈旧勘误 append source-review)。
- **阻塞或等待**: 等 Stage 13(及 P3-16 各源)线上 Daily run 验证。`.claude/stage-briefs/` 存有历批 outline/brief(gitignored);P3-16 source-review = `docs/CHINA_MACRO_LIQUIDITY_PROPERTY_SOURCE_REVIEW.md`。
