...
| Cache version | `stage-70city-detail-2` |
...
- **当前进行中**: 无活跃任务。70 城城市明细折叠(`e03c36a`)+ 明细区字体对齐 .note(`3433326`,bump `stage-70city-detail-2`)均已上线,**tierBreakdown 线上验证通过**(一线/二线/三线各和=4/31/35,一线新房上涨=上海/广州/深圳)。Stage 10 / UX / hotfix 均已收口。
- **下一步建议**: ① 触发 Daily 验证 tierBreakdown;② 明细折叠区目前默认样式(未加 CSS),如需美化另开小 stage;③ P3-16 余源 **Stage 11 = OMO(PBOC `pbc.gov.cn`,新域名 US runner 可达性需 Codex 实证)** → 社融分项 → MLF。owner 定下一步优先级。
- **阻塞或等待**: 等 70 城明细 stage 线上 Daily run 验证。`.claude/stage-briefs/` 存有历批 outline/brief(gitignored);P3-16 source-review = `docs/CHINA_MACRO_LIQUIDITY_PROPERTY_SOURCE_REVIEW.md`。
