...
- **验证**：新增模拟 401/402/403/429 的零真实请求回归，检查旧摘要保留、日期不变、单次请求、凭据/正文不泄漏；纳入现有 Cloud 检查入口。`check:changed` 执行完整 `check:all` exit 0；最终 Cloud 专项复验、文档链接与 `git diff --check` exit 0。独立工作区保留改动，等待账户实测与串行集成，未发布。
- **尚未完成**：账户登录待用户完成，真实账户期限、余额、豁免和新功能样本尚未核实。试用最多 12 个普通只读请求（预计不超过 12 QU），只在账户确认免费可用后串行执行；不创建 Situation、不运行研究代理/Brief、不创建周期 Monitor，不启用收费超额。遇权限或余额不明立即停止，不重试、不自动购买。
