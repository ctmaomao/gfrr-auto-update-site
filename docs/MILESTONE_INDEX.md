...
| # | 一句话 | 主要 doc |
|---|---|---|
| **M-73** | Null-zero display guard: missing BGCR/TGCR repo spread and ICE Brent diagnostic null values no longer render/classify as `+0.0bp` / `0.00`;adds `check:null-zero-display-guards`;`check:all` 74 | `docs/PROJECT_BACKLOG.md` |
| **M-72** | Macro-driver date display fix: JOLTS / CRE vintage formatter no longer produces `undefined` / `NaN` / `QNaN`;frontend cache `28.0M-72V` | `docs/PROJECT_BACKLOG.md` |
