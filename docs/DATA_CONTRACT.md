...
v28.0J-2B post-deploy audit 已通过，当前 live data 已包含 `aiInterpretationLayer.contractVersion = v28.0J-0`。当前前端 asset cache 版本以 `scripts/app.js` 的 `APP_VERSION` 为准（现 `audit-hardening-1`）。
...
audit-hardening-1 Frontend Asset Cache Busting 只定义前端静态资源版本契约，不改变数据契约、Worker runtime、Brent promotion、sourceProbe、secondary diagnostics、KV 或 `data/*.json` / `realtime/*.json`。本轮触发原因是 External AI 前端 fail-closed freshness/contract 加固与 ODP 渲染失败隔离；cache busting 用于避免浏览器沿用旧 module graph。

当前前端资源 cache 版本以 `scripts/app.js` 的 `APP_VERSION` 为准（现 `audit-hardening-1`）。

要求：

- `index.html` 入口 module script 必须指向 `app.js?v=audit-hardening-1`。
- `scripts/app.js` 与当前前端入口实际加载的 `scripts/modules/*.js` 本地相对 `.js` import 必须使用 `?v=audit-hardening-1`；M-94 后有意冻结且当前未接入的 `scripts/modules/realtime.js` 不属于当前前端 runtime 入口,其 import query 不应随当前 asset bump 更新,由 `check:realtime-js-frozen` 守住。
- 核对线上版本:看 `scripts/app.js` init 时的 console 行 `[app] … APP_VERSION=<版本>`(当前 `audit-hardening-1`),或检查已加载的 `app.js?v=…` URL token;两者须与 `?v=` 一致。
...
```bash
node scripts/bump-frontend-asset-version.mjs audit-hardening-1
npm run bump:frontend-asset-version -- audit-hardening-1
```

该工具用于以后前端 HTML / JS 改动时统一 bump cache version。当前正式版本仍是 `audit-hardening-1`；它只更新前端 asset version、contract 和相关文档，不访问网络、不写 KV、不写 `data/*.json` / `realtime/*.json`、不 deploy Worker。Worker runtime 改动不需要 bump frontend asset version，除非同时改 `index.html`、`scripts/app.js` 或当前入口实际加载的 `scripts/modules/*.js`。
...
- `summary`:`red_count / yellow_count / green_count / primary_score_pct / primary_score_basis / red_pct / weighted_risk_score / stage_score / stage_label(_en) / trigger_score / trigger_label(_en) / momentum / category_scores / similarity / verdict_label(zh) / verdict_label_en / verdict_desc / verdict_desc_source / narrative_plan`。**页面与判读主分数固定为 `primary_score_pct = red_pct = 红灯数 ÷ 指标总数 × 100`**;`primary_score_basis` 必须为 `red_light_ratio`。`weighted_risk_score=(红+0.5黄)/指标总数×100` 只是黄灯调整辅助压力分,可用于趋势辅助,不得作为阈值分档、Hero 主分数或 `verdict_desc` 的本周主判读分。Stage/Trigger 以轴内红/黄/绿 = 100/50/0 取均值;stage 四档为 30/50/70,trigger 四档为 25/45/65。`momentum` 按相邻状态迁移级数计算;`similarity` 使用 reference 2026-07-06 历史校准表,本地独有 `spy_vs_rsp_6m` / `token_revenue_ratio` 在取得 reviewed 历史类比前不进入相似度分母。`verdict_desc_source` 当前为 `bubble-watch-narrative-v1`;`narrative_plan` 必须来自本地 `local_indicator_evidence_pack`,按 scorecard / market_structure / capital_fundamentals / credit_macro / override_conclusion 组织证据。`verdict_desc` 须保持参考页同量级(当前 checker 锁 900-2600 UTF-8 bytes),不得膨胀为逐指标明细。生产正文不得直接采纳上游 `summary.verdict_desc`;上游历史 snapshots 只允许作为写作结构校准与回归评估参考。
- `scoring`:`base_tier`(red_pct 四档 25/40/60)、`effective_tier`、`override_active`、`override_rule(s)`、`two_axis_upgrade`、`resonant_categories`。主分仍是 red_pct;升级顺序为分类共振(红灯占比 ≥50% 的分类 ≥2 个 → 至少高风险预警),再检查两轴共振(stage ≥60 且 trigger ≥50 → 至少高风险预警;stage ≥60 且 trigger ≥65 → 系统性顶部)。checker 全量 replay,**阈值与升级规则不得悄改**。
- `indicators[27]`:固定 id 集(见 `scripts/check-bubble-watch.mjs` EXPECTED_IDS),每项 `axis(stage|trigger) / category / name_en / name_zh / status(red|yellow|green) / value_display / note / threshold_text / source_name / as_of / stale / provenance`。`provenance.mode ∈ auto | curated | auto_fallback`;curated/auto_fallback 必带 `asOfDate` + `maxAgeDays`,`stale` 与超期状态机器一致;页面以 `as_of` 显示单卡日期,未过期但早于本期时标「沿用」。
- `market_technical_heat`:独立「公开市场技术热度」审计子面板,`contractVersion = "bubble-watch-market-technical-heat-v1"`;只读 `Yahoo Chart v8` 免费公开日线价格,构造 NVDA/AMD/MSFT/GOOGL/META/TSLA/AVGO/ORCL 等权 AI 篮子并与 QQQ/SPY 对照,固定 5 项审计项(`relative_momentum_21d` / `rsi_14d` / `bollinger_pct_b` / `sma_200_deviation` / `correlation_beta_60d`)。该字段必须声明 `display-only` 与 `excluded from 27-indicator`,**不得进入 `summary.red_count/yellow_count/green_count`、`primary_score_pct`、`weighted_risk_score`、`scoring`、`verdict_desc` 主判读、GFRR scoring/decision/execution/position 或任何交易建议**。`source_priority` 必须按 Yahoo Chart 免费公开主源 -> public-apis Finance 候选(多为需 key 的 source-review fallback) -> Wind paid final fallback 的顺序披露;免费源不可用时可 fail-closed 为 `status="unavailable"` 且不改变主分数。
