...
- `index.html` 入口 module script 必须指向 `app.js?v=frontend-stale-static-wire-3`。
- `scripts/app.js` 与 `scripts/modules/*.js` 的本地相对 `.js` import 必须使用 `?v=frontend-stale-static-wire-3`。
...
```bash
node scripts/bump-frontend-asset-version.mjs frontend-stale-static-wire-3
npm run bump:frontend-asset-version -- frontend-stale-static-wire-3
```

该工具用于以后前端 HTML / JS 改动时统一 bump cache version。当前正式版本仍是 `frontend-stale-static-wire-3`；它只更新前端 asset version、contract 和相关文档，不访问网络、不写 KV、不写 `data/*.json` / `realtime/*.json`、不 deploy Worker。Worker runtime 改动不需要 bump frontend asset version，除非同时改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js`。
