...
编辑/研究类 11 项是 **curated-origin** 而非全部永久人工:人工快照来源仍为 `config/bubble-watch-curated.json`(改 value/status/note + asOfDate 后触发 workflow),但 `config/bubble-watch-source-candidates.json` 必须覆盖 11 项免费公开源候选。当前 `vc_ai_share`、`ai_ipo_pipeline`、`accounting_events`、`ceo_hedging` 为 `hybrid_live`,build 先尝试 Crunchbase News / SEC-DOJ public search / GDELT 等公开源自动覆盖;成功时 `provenance.mode=auto`,失败时 fail-closed 沿用 curated 快照并记录 `fetch_failures`。其余 7 项为 `candidate_only`,只登记免费候选源与限制,不得在未 source-review 前自动改灯。自动 12 项抓取失败同样 fail-closed 沿用 curated 快照并标注「实时抓取失败,沿用 YYYY-MM-DD 快照」。
