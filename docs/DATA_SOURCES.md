...
| **DeepSeek — Bubble Watch weekly editorial** | 复用仓库 DeepSeek chat-completions endpoint，默认 `deepseek-v4-flash` | 将已校验 compact evidence pack 编辑为周度长篇只读判读 | `response_format=json_object`、`max_tokens=5000`、timeout 120s、每 workflow 最多一次调用且 retry=0。Provider 不浏览网页；output 必须通过引用完整性、unsafe wording、评分边界和质量审阅后才可由专用 writer 写入 `summary.weekly_editorial`。失败时保留 `bubble-watch-narrative-v2` fallback |
