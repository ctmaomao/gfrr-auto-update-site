...
代理源置信度校准: `insider_sell_buy` / `ai_ipo_pipeline` / `capex_reaction` / `ceo_hedging` / `token_revenue_ratio` / `enterprise_deploy` 的免费/付费代理源若更严重,必须先通过本地二次确认门槛,否则发布灯色按 `local_proxy_confidence_v1` 多源/样本规则降档。自动原始证据仍保留在 `provenance.detail.proxyConfidenceCalibration`(兼容别名:`templateCompatibilityCalibration`),并在 `meta.proxy_confidence_calibrations[]` 汇总;这不是抓取失败,也不是人工覆盖生产 JSON,目的是避免单一新闻/搜索/平台 proxy 把 Bubble Watch 总分误推高。上游/curated 只可在 `maxAgeDays` 内作为显示值锚点,不是判级权威;若上游长期不可达,本地规则仍独立工作,过期锚点按 STALE 机制暴露。
