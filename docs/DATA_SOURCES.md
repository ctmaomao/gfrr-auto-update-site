...
编辑/研究类 11 项改称 **curated-origin**:源候选矩阵登记在 `config/bubble-watch-source-candidates.json`。其中 `vc_ai_share` / `ai_ipo_pipeline` / `token_volume_mom` / `enterprise_deploy` / `accounting_events` / `ceo_hedging` 已为 `hybrid_live`,build 先尝试免费公开源自动覆盖,失败仍 fail-closed 回 `config/bubble-watch-curated.json`;`dc_abs_spread`、`neocloud_credit`、`token_revenue_ratio`、`arr_2nd_deriv`、`capex_reaction` 仍为 `candidate_only`,只登记免费候选源与限制,不得强行自动改灯。所有自动指标 fail-closed 回退 curated 快照。契约见 [`DATA_CONTRACT.md`](DATA_CONTRACT.md)「bubble-watch 专题数据契约」。
