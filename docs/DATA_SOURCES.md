...
| **SaaStr WordPress public API** | `saastr.com/wp-json/wp/v2/...` | `arr_2nd_deriv` 的现行 hybrid public-source runtime；Sacra仅候选见下一行 | 公开 AI ARR/run-rate 里程碑 proxy；builder 要求多期 Anthropic 里程碑可解析，并用最新里程碑自身日期对照该指标 `maxAgeDays`。公司、金额与ARR/annualized run-rate口径必须直接绑定；疑问/条件/否定/前瞻或其他公司金额不得补位。已审明确标题优先；325206仅允许已审三短句结构，不硬编码金额。日历真实校验保持45天边界；网页本周抓取成功不等于底层观测新鲜；超龄时自动结果失败关闭并沿用更新的 curated 快照。该口径不是审计收入、不是私营公司完整 panel |
| **ARR independent-source candidate review** | `config/bubble-watch-arr-evidence.json` / `scripts/review-bubble-watch-arr.mjs` | `artifact_sanitizer_layer`；离线候选复核，未接生产 | 2026-09-07 owner批准接入改造；不代替供应商许可。Anthropic官方披露保留47B下限及5月观察区间，非65B融资；Sacra仅研究候选，自动访问须书面许可。固定本地输入、无网络/付费/生产写入；保守45天时效、不伪造精确日期、不混接估值/融资/产品收入，来源独立性仍须review。详见[来源权限及后续门槛](ARR_INDEPENDENT_SOURCE_REVIEW.md)；原SaaStr runtime/curated/源审计规则保持 |
