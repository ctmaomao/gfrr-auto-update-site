...
2026-09-18 研究证据诊断：owner 批准三项后续验收。新增离线 `analyze-pressure-evidence.mjs` 仅复用已有 15 源 cache/report 和真实 ledger；不增加取数、源、费用或评分输入。派生 artifact 保留逐周参考偏离、模块/通道记账、源日期与代理状态，不上传原始历史价格/利率序列。按 HY 原始/代理可用性分组仍非完整历史生产复现；参见[验收报告](PRESSURE_EVIDENCE_ACCEPTANCE_2026_09_18.md)。
...
2026-09-08 ARR 免费候选：[Epoch AI 来源评审与隔离核验](ARR_EPOCH_SOURCE_REVIEW.md)。#312 sanitizer、#313 固定官方有界读取/hash-only 比较、#314 固定 ignored 归档、#315 固定 GitHub 身份/祖先/摘要/ZIP 的跨 run 交接、#316 既有周一 schedule 的隔离 candidate job 均已独立审阅合并。candidate job 为 contents:read/actions:read，首次 schedule run 才读一个官方 CSV；hash-only artifact 保留 30 天，不含原始金额/日期/URL/Notes。仅从同 workflow 成功 schedule/main 的 30 天历史有界选择，完整身份校验后比较；缺失/过期 baseline_required，坏历史拒绝、不偷偷退更旧版本。最多 9 个应用 GET，元数据/历史取回/CSV 三阶段各最多 15 秒，无应用重试；不含 Actions checkout/upload 平台传输。来源交接不认证收入事实，不维护批准基线/latest，不因下载更新观测日期或资格。#316 CI `34214841758`、Pages `34215725692` 已成功；真实 scheduled 上传/跨 run 下载仍待后续周一验收，不能以 CI/mock 代替。#317 已逐条复核 18 行及七篇官方公告，发现插值、日期上界及金额限定，尚不能建立可靠四点斜率；不改变 SaaStr/Core-23、生产值、付费调用或既有刷新 job。观测精度/金额限定仍 unknown/hold、productionEligible 恒 false；45 天底层时效及生产切源门槛保留，不购买或绕过 Sacra。
...
| **stockanalysis.com** | `/stocks/nvda/statistics/`、`/etf/spy/holdings/`、`/stocks/*/financials/{,cash-flow-statement/}?p=quarterly`、`/stocks/{msft,amzn,googl}/metrics/`、`/stocks/orcl/financials/metrics/?p=quarterly` 公开 HTML | NVDA 远期 PE、S&P Top-5 权重(SPY 持仓代理,服务端只渲染前 ~25 行)、**EDGAR 被封时的季报镜像**(OCF/Capex/Revenue/FCF,~20 季服务端渲染)、Cloud RPO 二级源(MSFT/AMZN/GOOGL 季度 operating metrics + ORCL 季度 metrics;按实际期末匹配同比,缺失不跳位) | 公开页代理;Top-5 是 SPY 持仓口径非 S&P 官方权重;季报数字为 $M 口径镜像非 SEC 原始 filing;Big5 capex/OCF 主口径固定为 AMZN/MSFT/GOOGL/META/ORCL realized TTM cash capex ÷ Operating Cash Flow,同一 `mag4_fcf_yoy` score slot 改造而来;不得与前瞻 FCF、levered/unlevered FCF、单家公司压力或参考站编辑口径混用;RPO metrics 标注为 StockAnalysis/Fiscal.ai 镜像,不是公司官方 API |
...
| **SaaStr WordPress public API / Sacra public research** | `saastr.com/wp-json/wp/v2/...`、`sacra.com/research/...` | `arr_2nd_deriv` 的 hybrid public-source 覆盖 | 公开 AI ARR/run-rate 里程碑 proxy；builder 要求多期 Anthropic 里程碑可解析，并用最新里程碑自身日期对照该指标 `maxAgeDays`。公司、金额与ARR/annualized run-rate口径必须直接绑定；疑问/条件/否定/前瞻或其他公司金额不得补位。已审明确标题优先；325206仅允许已审三短句结构，不硬编码金额。日历真实校验保持45天边界；网页本周抓取成功不等于底层观测新鲜；超龄时自动结果失败关闭并沿用更新的 curated 快照。该口径不是审计收入、不是私营公司完整 panel |
...
2026-09-10 Oil News 调用策略：429 直接退出当前请求，复用既有 24h classified cooldown；不进行秒级限流重试。timeout/network/5xx 仍最多一次有界重试，其他 GDELT 调用者不变。6h workflow cadence、24h fresh TTL、72h stale fallback 与来源资格隔离保持原样；这项调整减少无效请求，不代表上游可用性已经恢复。
...
2026-09-18 owner 已明确批准首次验收及每周自动运行。[ADR-0055](ADR/0055-acled-weekly-automation.md)限定新 workflow 的预算、持久防重复、私有原件、自动 provenance 和双配置发布/部署验收；仅该 reviewed 路径替代下文旧 manual-only 获取限制。该批准不是 ACLED 官方官网自动访问许可；EULA §3.3 限制仍披露。旧提醒、API/HAPI 与一次预算不变，policy PR 本身不下载或上线。
...
2026-09-18 云端整批验收按[ADR-0054](ADR/0054-acled-private-batch-acceptance.md)独立一次26请求执行，`acled-private-batch-acceptance.yml`默认dry-run、只读权限、无schedule/artifact。CLI复核执行上下文，退出确认后才运行隔离标准化，输出`validated_not_published`仅说明两类sanitizer及清理通过；不更新生产配置或将preparedBy/source字段伪装成自动生产来源。[唯一 run 35277723078](https://github.com/ctmaomao/gfrr-auto-update-site/actions/runs/35277723078)已成功：十二文件、26请求，周991218行/09-05、月43588行/09-11，退出与清理确认；本次预算已耗用，持续下载及发布尚未启用。验收发现手工周解析的本地时区偏移，现保留 Excel 日期序号并使用既有 UTC 转换；UTC/Auckland/Los Angeles 真实解析回归及旧选项负对照保护此修复，不直接重写旧生产日期。

认证批次与私有标准化组件：`acled-session-batch.mjs`复用固定详情及批次限额，最多登录1/详情12/文件12/退出1；只在退出确认后返回完整buffers。`acled-private-validation.mjs`仅在唯一临时工作区运行现行两个sanitizer，不改生产，子进程不继承账号凭证，原件不进入artifact/cache/Git；清理失败不交候选。原无入口阶段已由上文一次验收CLI/workflow替代，持续调度/发布边界仍保持独立评审。
...
最新[地理及历史版本核验](ACLED_HDX_MONTHLY_SOURCE_REVIEW.md#地理覆盖与历史版本证据2026-09-16)：`review:acled-geography` 仅离线复验私有年度档案；36组名称/代码对吻合，24个有限参考国家代码未在admin0返回、8名称仍未解决，不证明全球或领土口径等价。OCHA来源分admin0/admin2，未证明互斥，禁止直接相加。已找到08-28历史metadata，但与09-04共用可变download URL，历史正文未取得。AFG/admin2新小样本预算另行询问，未执行、不借用既有预算；不改变生产来源。
...
后续免手工目标使用[逐指标替代验收](ACLED_HDX_MONTHLY_SOURCE_REVIEW.md#替代验收与免手工目标2026-09-16)：新增零网络报告及规范化PV逐行比较核心；当前真实引用缺失，原24月不覆盖年度四年基线。独立批准的[一次年度验收](ACLED_HDX_MONTHLY_SOURCE_REVIEW.md#年度候选与公开筛选规则2026-09-16)已用4请求/3,723,475字节取得218返回国家代码的2022–2025完整48月私有候选，零网络复验通过；不扩四槽预算、不自动重跑。现可用 `review:acled-replacement -- --annual` 显式hash复验并接入年度时间覆盖，数值/全球/生产状态仍未通过。两份本地PV原表08-21与候选08-28不同版，250原表名称与218候选代码只有182精确同名，尚非核准映射，不执行跨版本等价判断。官网已说明两死亡产品定义，但与HAPI实际映射、全球覆盖、同版本逐行数值及免费周表自动渠道仍未证明。已将既有ARR本地heartbeat合并为双任务跟进，不改GitHub提醒/生产来源；本次未发邮件。
...
Monthly trend uses the 24 consecutive calendar months before the real `asOfDate` month, including exclusion of month-end releases' own month. Any missing month makes the existing whole trend `null`; observed zero is preserved. This is calendar coverage, not proof of complete country coverage ([ADR-0034](ADR/0034-acled-complete-month-windows.md)). Monthly source date, annual fields and scoring remain unchanged by this definition correction.
...
2026-09-07 PR #307 合并后的下一刀引入 telemetry v4，见
[ADR-0030](ADR/0030-web-ngrams-event-support-provenance.md)：在原方向/轴/bucket/
时间/域名条件外增加标题内同地点、对象族、事件类型及有明确船名时的身份匹配；
缺失或歧义不计支持。同 URL/标题哈希仅算 discovery overlap，Web 同标题只选
一个支持代表，reference 转载只选一个实际出版 URL，跨 provider 必须有不同
故事和独立域。ignored observation 增加脱敏 reference 表、支持链接及拒绝计数；
不保存原文、URL、船名或非法日期原值，生产 cache 仍无逐篇字段。v2/v3 保留严格
验证及历史审计，质量门只统计 v4；原30天/120样本、阈值、全候选分母及人工切源
