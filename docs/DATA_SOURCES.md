...
2026-09-08 ARR 免费候选：[Epoch AI 来源评审与隔离核验](ARR_EPOCH_SOURCE_REVIEW.md)。#312 sanitizer、#313 固定官方有界读取/hash-only 比较、#314 固定 ignored 归档、#315 固定 GitHub 身份/祖先/摘要/ZIP 的跨 run 交接均已合并。owner 现授权实施既有周一 schedule 的隔离 candidate job：contents:read/actions:read，首次 schedule run 才读一个官方 CSV；hash-only artifact 保留 30 天，不含原始金额/日期/URL/Notes。仅从同 workflow 成功 schedule/main 的 30 天历史有界选择，完整身份校验后比较；缺失/过期 baseline_required，坏历史拒绝、不偷偷退更旧版本。最多 9 个应用 GET，元数据/历史取回/CSV 三阶段各最多 15 秒，无应用重试；不含 Actions checkout/upload 平台传输。来源交接不认证收入事实，不维护批准基线/latest，不因下载更新观测日期或资格。新接入 PR 尚待独立审阅与合并、真实 scheduled 上传/跨 run 下载验收；不改变 SaaStr/Core-23、生产值、付费调用或既有刷新 job。观测精度/金额限定仍 unknown/hold、productionEligible 恒 false；逐条复核和生产切源继续另审，不购买或绕过 Sacra。
