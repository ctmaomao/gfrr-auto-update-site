...
| **Wind MCP paid optional** | `mcp.wind.com.cn/vserver_{analytics,economic,financial_docs}_data/mcp/` | `dc_abs_spread` 的 paid optional 覆盖;`ceo_hedging` 的 paid fallback-only 覆盖 | 需 `WIND_API_KEY`;builder 只发布派生判级/摘要,不提交 raw Wind 响应。`dc_abs_spread` 使用数据中心 ABS 样本识别 + 中国 ABS AAA 收益率基准方向 + 金融新闻证据,并明确标注为 paid proxy;样本券专属估值利差字段为空时不得写成正式数据中心连续利差。`ceo_hedging` 仅在 GDELT 免费源 429/超时/不可用后调用 Wind 新闻兜底。Cloud RPO 已有 StockAnalysis/Fiscal.ai 免费二级源;Wind announcement/fundamental 路径仅作为人工排查备选,不得自动付费改灯 |
...
| **Bubble Watch source-health audit** | `.github/workflows/audit-bubble-watch-sources.yml` / `scripts/audit-bubble-watch-sources.mjs` | 周二至周五免费源链路健康检查;artifact-only report | `permissions: contents: read`;运行 builder 后恢复 `data/bubble-watch*.json` 与 `config/bubble-watch-curated.json`,只上传 `manual-artifacts/bubble-watch-source-health-*` artifact。scheduled 默认禁用 Wind;`dc_abs_spread` 无 Wind key、`ceo_hedging` 免费 GDELT 失败后 Wind fallback 被禁用,均记为 expected paid-fallback WARN,其它 fallback/stale/fetch failure 使 audit 失败 |

编辑/研究类 11 项改称 **curated-origin**:源候选矩阵登记在 `config/bubble-watch-source-candidates.json`。其中 `vc_ai_share` / `ai_ipo_pipeline` / `neocloud_credit` / `token_volume_mom` / `token_revenue_ratio` / `arr_2nd_deriv` / `enterprise_deploy` / `accounting_events` / `capex_reaction` / `ceo_hedging` 已为 `hybrid_live`,build 先尝试免费公开源自动覆盖;其中 `ceo_hedging` 只有在 GDELT 免费源失败后才尝试 Wind 付费新闻兜底,失败仍 fail-closed 回 `config/bubble-watch-curated.json`。`dc_abs_spread` 为 `hybrid_paid_optional`,有 `WIND_API_KEY` 时尝试 Wind 付费代理覆盖,无 Key/证据不足时同样 fail-closed。多地区免费候选源仍覆盖欧洲 Vantage data-center ABS、中国/香港 GDS 与上交所债券信息、L&G/IMF/CRAI 公开研究、HKEX/SGX/CNInfo/EastMoney 披露/新闻入口,只作校准与 fallback 证据;不得把 Wind proxy 或公开叙事材料伪装成正式数据中心 ABS 连续利差。所有自动指标 fail-closed 回退 curated 快照。契约见 [`DATA_CONTRACT.md`](DATA_CONTRACT.md)「bubble-watch 专题数据契约」。
