...
| PBOC 社融组件分项 | `pbc.gov.cn` 金融统计数据报告 | 社融存量同比 + 年内累计增量 + 8 个累计分项 evidence | implemented (Stage 12);audit-only/display-only;report-level cumulative, not loan-level micro;no derived monthly increment |
