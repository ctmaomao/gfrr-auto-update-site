...
P-score-43 起,新增 `npm run monitor:transport-shock-confirmation-factor-score-integration-preflight` 作为 **manual/local score-integration preflight monitor**。它只运行 P-score-38 preflight,输出 schema `transport-shock-score-integration-preflight-monitor-p43` 到 ignored `manual-artifacts/transport-shock-confirmation-factor/score-integration-preflight-monitor-latest.json`。状态 `blocked_on_external_evidence_or_source_rights` 表示剩余 blocker 已不是可继续 code-only 清理的事项:route freight 需要合法路线级油轮运费来源/授权或另开 reviewed policy change,high-frequency physical 需要真实 Oil Thermal/设施重复升高观测或另开 reviewed physical-proxy policy change。该 monitor 不联网、不写 production data、不接 workflow/Worker/frontend、不批准 score write、ODP finalBias、Brent promotion、今日总判断打分或 cross-validation。
