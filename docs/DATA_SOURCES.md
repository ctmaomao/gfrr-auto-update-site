...
编辑/研究类 11 项改称 **curated-origin**:源候选矩阵登记在 `config/bubble-watch-source-candidates.json`。其中 `vc_ai_share` / `ai_ipo_pipeline` / `neocloud_credit` / `token_volume_mom` / `token_revenue_ratio` / `enterprise_deploy` / `accounting_events` / `ceo_hedging` 已为 `hybrid_live`,build 先尝试免费公开源自动覆盖,失败仍 fail-closed 回 `config/bubble-watch-curated.json`;`dc_abs_spread`、`arr_2nd_deriv`、`capex_reaction` 仍为 `candidate_only`,只登记免费候选源与限制,不得强行自动改灯。多地区候选源已覆盖欧洲 Vantage data-center ABS、中国/香港 GDS 与上交所债券信息、HKEX/SGX/CNInfo/EastMoney 披露/新闻入口;但只要缺少连续可比数值或可复算事件窗口,仍不得自动改灯。所有自动指标 fail-closed 回退 curated 快照。契约见 [`DATA_CONTRACT.md`](DATA_CONTRACT.md)「bubble-watch 专题数据契约」。
