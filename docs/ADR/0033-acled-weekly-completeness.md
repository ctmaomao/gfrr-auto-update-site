...
Post-merge Pages run `34313341031` exposed a test-only dependency assumption: its no-install environment could not start the fixture sanitizer's static XLSX import, although PR CI had installed dev dependencies. The admission fixture now lives in the OS temporary directory with an explicit test-only parser stub that throws on any workbook read. It still executes the actual checker/sanitizer CLI and asserts pre-parse rejection and byte preservation. This is not real workbook parsing acceptance. Production imports, dependency locks, checkers and the Pages no-install policy remain unchanged; no test is skipped. Verify the full suite in a clean worktree without `node_modules` and the subsequent Pages deployment.
