...
## 2026-06-26 Clarification: rule-based layer vs live external layer

Decision point 3 described the pre-production / disabled-provider state and
must not be read as a statement about the current production
`externalAiInterpretationLayer`.

Current production has two separate interpretation objects:

1. `aiInterpretationLayer` is the rule-based interpretation layer. It remains
   `generatedByExternalAi=false`, `usesExternalAiApi=false`, and does not call
   DeepSeek, OpenAI, or any external AI provider.
2. `externalAiInterpretationLayer` is the visible read-only external AI
   display layer. It is generated by the approved
   `External AI Production Refresh` workflow, may use DeepSeek, and remains
   guarded by production contract validators and quality review fields.

This clarification does not change the hard boundary: neither layer may affect
scoring, `decisionModel`, `executionLock`, `positionGuidance`, Action Queue,
Trigger Monitor, or Invalidation Rules.
