...
- **验收与限制**：新增 [离线测试](../tests/unit/epoch-arr-weekly.test.mjs) 覆盖 mock 全链路、baseline-required、修订、历史/当前身份、截断、时效、错误脱敏、超时和工作流隔离；归一化 production job 摘要锁定 `d3568a8f` 内容，之后若需改生产 job，另行审阅并更新该窄范围约束。本轮不触发真实 CSV/production refresh 或上传，真实 scheduled 上传/下次跨 run 下载仍待本 PR 审阅合并后验收，不能把 mock 成功写成全自动已上线。
