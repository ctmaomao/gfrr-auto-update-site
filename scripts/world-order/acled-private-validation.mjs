...
export function validateAcledPrivateBatch(workbooks) {
  ...
  try {
    ...
    entries = validateAcledDownloadManifest(workbooks.map(w => w.url)).entries;
  ...
  try {
    ...
    for (const kind of ['weekly', 'monthly']) fs.mkdirSync(path.join(directory, 'manual-artifacts', 'world-order', 'acled-input', kind), { recursive: true });
