...
export const SCOPE = Object.freeze({ id: 'acled-admin2-scope-20260916', requests: 3, bytes: 8 * 1048576,
  rows: 10002, sampleRows: 10000, timeoutMs: 15000, spacingMs: 1100, storageBytes: 8 * 1048576 + 65536 });
...
function parse(text, cap = SCOPE.bytes) {
...
function inspectRows(text) {
  ...
  for (const row of rows) {
    ...
    if (typeof row.location_code !== 'string' || !/^[A-Z]{3}$/u.test(row.location_code) || row.admin_level !== 2
      || row.resource_hdx_id !== RESOURCE || row.event_type !== 'political_violence') fail('row_scope');
    for (const key of ['location_name', 'admin1_code', 'admin2_code', 'admin1_name', 'admin2_name'])
    ...
    const country = row.location_code;
    bind(`c:${country}`, row.location_name); countries.set(country, row.location_name);
    bind(`p:${country}:${row.admin1_code}`, row.admin1_name);
    bind(`d:${country}:${row.admin2_code}`, JSON.stringify([row.admin1_code, row.admin2_name]));
    const reverseEntries = [[`c:${row.location_name}`, country], [`p:${country}:${row.admin1_name}`, row.admin1_code], [JSON.stringify([country, row.admin1_code, row.admin2_name]), row.admin2_code]];
    ...
    const key = JSON.stringify([country, row.admin1_code, row.admin2_code]);
    if (seen.has(key)) fail('duplicate_row'); seen.add(key); provinces.add(`${country}:${row.admin1_code}`);
  }
  return { validRows: rows.length, returnedAdmin1Codes: provinces.size, returnedAdmin2Codes: seen.size,
    returnedCountryCodes: [...countries.keys()].sort(), scope: 'returned_only_not_global_universe', month: '2025-01', layer: 2, rowLimitHit: false, geographicCompleteness: 'not_proven',
    administrativeCodes: 'source_declared_not_authoritatively_verified', nationalAggregation: 'not_performed' };
}
export function inspectScopeSnapshot(snapshot, now = new Date().toISOString()) {
  ...
  if (texts.some(v => typeof v !== 'string') || texts.reduce((n, v) => n + Buffer.byteLength(v), 0) > SCOPE.bytes) fail('body_size');
...
export async function collectScope(contact, baselines, deps = {}) {
  ...
  try {
    ...
    async function request(stage, params, rowCap) {
      if (report.requestCount >= SCOPE.requests) fail('request_budget');
      await wait(Math.max(0, SCOPE.spacingMs - (tick() - lastStart))); lastStart = tick();
      report.requestCount++; const call = { stage }; report.calls.push(call);
      const controller = new AbortController(); let timer, reader;
      const deadline = new Promise((_, reject) => { timer = setTimeout(() => { controller.abort(); reject(new Stop('timeout')); }, SCOPE.timeoutMs); });
      try {
        return await Promise.race([deadline, (async () => {
          ...
          const response = await fetcher(new URL(`https://hapi.humdata.org/api/v2/${route}?${new URLSearchParams(params)}`), {
            redirect: 'manual', signal: controller.signal, headers: { Accept: 'application/json',
              'User-Agent': 'GFRR/isolated-admin2-scope-acceptance', 'X-HDX-HAPI-APP-IDENTIFIER': identifier } });
          ...
          if (length !== null && (!/^\d+$/u.test(length) || Number(length) > SCOPE.bytes - report.totalBytes)) fail('byte_budget');
          ...
          while (true) {
            ...
            if (report.totalBytes > SCOPE.bytes || (stage !== 'sample' && size > 65536)) fail('byte_budget');
            chunks.push(value);
          }
          ...
          if (rows.length > rowCap || report.totalRows > SCOPE.rows) fail('row_budget');
