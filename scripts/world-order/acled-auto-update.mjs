...
export async function runAcledAutoUpdate({ execute = false, env = process.env, root = process.cwd(), now = new Date(),
  ...
  try {
    ...
    const slot = env.GITHUB_EVENT_NAME === 'schedule' ? acledWeekSlot(now) : 'initial';
    report.expectedHead = expectedHead;
    report.claim = await claimAcledSlot({ slot, expectedHead, token, fetchImpl });
    ...
    if (batch.report.status !== 'authenticated_zip_batch_read' || batch.report.logout !== 'confirmed'
      || batch.report.sessionMayRemain !== false || !batch.workbooks) return report;
    validated = validate(batch.workbooks); batch.workbooks = null;
