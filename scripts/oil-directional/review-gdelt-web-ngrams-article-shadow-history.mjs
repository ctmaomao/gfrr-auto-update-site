...
export function evaluateWebNgramsShadowHistory(samples, policy) {
  ...
  return {
    ...
    requiredNextStep: qualityGatePassed
      ...
      : 'original publication time remains unresolved; metadata support cannot qualify; retain unchanged 30-day/120-sample policy and legacy history',
    productionImpact: {
...
function writeGitHubSummary(review) {
  ...
  const summary = [
    ...
    `- V5 quality usable samples: **${review.qualityMetrics.usableSampleCount}**`,
    `- V5 quality observation days: **${review.qualityMetrics.observationDays}**`,
    '- Gates use only v5; original publication time is unresolved and metadata support cannot qualify. Thresholds and source mode are unchanged.',
...
function main() {
  ...
  if (options.printJson) console.log(JSON.stringify(review, null, 2));
    ...
    console.log(`v5QualityUsableSamples: ${review.qualityMetrics.usableSampleCount}`);
    console.log(`v5QualityObservationDays: ${review.qualityMetrics.observationDays}`);
