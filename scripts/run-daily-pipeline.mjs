...
function buildFallback() {
  ...
  next.summary = 'v26.0B 日构建已退回到上次有效慢变量结构。';
  next.recovery = {
    degradedMode: true,
    safeOutput: true,
    lastRun: isoNow,
...
function build() {
  ...
  const data = {
    ...
    topRisks,
    decisionLine: `当前已进入 v26.0B 交易引擎模式：实时快变量 ${realtime.sourceMode}，执行状态灯为 ${lock.levelLabel}。先看状态灯，再决定能不能动。`,
    summary: `v26.0B 正根据混合实时架构输出交易引擎结论。最新快变量：布伦特 ${risk.brent.toFixed(1)}、美元 ${risk.dxy.toFixed(2)}、VIX ${risk.vix.toFixed(2)}、HY OAS ${risk.hy.toFixed(2)}%。`,
    ...
    recovery: {
      ...
      safeOutput: true,
      lastRun: isoNow,
      notes: realtime.notes || ['v26.0B 慢变量已由最新 realtime 快照重算。']
    },
    tradingSystem: {
      signalEngine: {
        strength: risk.score,
...
console.log('Built v26.0B radar data successfully.');
