...
const MONITOR_VERSION = 'transport-shock-production-refresh-monitor-p9';
...
function classifyStatus(energyTransport) {
  if (!isPlainObject(energyTransport)) return 'payload_missing_energy_transport';
  if (energyTransport.transportShockCandidate === undefined) return 'awaiting_production_refresh';
...
function createMonitorResult(options) {
  ...
  const candidate = energyTransport?.transportShockCandidate;
  const status = classifyStatus(energyTransport);
