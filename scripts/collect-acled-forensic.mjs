...
import { FORENSIC, runForensicAcceptance, reviewForensicEvidence } from './world-order/acled-admin2-forensic.mjs';
...
async function main() {
  ...
  if (!args.length || (args.length === 1 && args[0] === '--dry-run')) {
    console.log(JSON.stringify({ status: 'dry_run', budget: FORENSIC, scope: 'all_returned_countries', month: '2025-01', layer: 2, networkRequests: 0, productionEligible: false })); return;
  }
  ...
  execFileSync('git', ['check-ignore', '--quiet', '--', path.join(root, 'manual-artifacts', FORENSIC.id, 'probe.private.json')], { cwd: root, stdio: 'ignore' });
  const now = new Date().toISOString();
  if (args[0] === '--review') { console.log(JSON.stringify(await reviewForensicEvidence(root, now))); return; }
  ...
  const report = await runForensicAcceptance(root, contact, baselines);
  console.log(JSON.stringify(report)); if (report.status !== 'evidence_saved_not_candidate') process.exitCode = 1;
}
