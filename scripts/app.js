...
function buildSourceSummary(sourceDetails = {}, sourceStatus = {}) {
  ...
  if (!entries.length) {
    ...
    if (statusEntries.length) {
      ...
      return {
        ...
        issueLines: statusEntries.slice(0, 4).map(([key, status]) => `${key}: ${String(status).replace('fallback', '回退').replace('secondary', '次级来源').replace('failed', '失败')}`)
      };
    }
  ...
  entries
    ...
    .forEach(([key, detail]) => {
      issueLines.push(`${key}: 已启用回退来源 ${detail?.source || '次级来源'}`);
    });
...
function formatFreshnessLabel(level) {
  const map = {
    fresh: '新鲜',
    aging: '老化',
    stale: '陈旧',
...
const POSITION_RULES = Object.freeze({
  stateBandMap: {
    'Risk-On': {
      ...
      leveragePolicy: '避免激进杠杆；如已存在基础融资，仅允许维持。',
    ...
    Defensive: {
      ...
      newExposurePolicy: '暂停新增广义风险暴露，只允许防守性再平衡。',
...
function buildActionQueueFallback(data = {}, metadata = {}, strategyState = 'Caution') {
  ...
  const queue = {
    ...
    unblockConditions: defensiveFallback
      ? [
          '待近3日趋势转平或转负后，再恢复战术性再冒险。',
          '待共振数量降到 3 以下后，再恢复选择性加仓。',
          '仅在波动与流动性压力恢复正常后，才重新开放杠杆。',
          '仅在关键告警清除后，才重新开放提高集中度。'
        ]
      : [
          '仅在缓和倾向改善后，才允许更广泛地加仓。',
          '仅在短期恶化停止后，才允许战术性再冒险。',
    ...
    escalationHint: defensiveFallback ? '若压力指标恶化或数据质量继续下降，应升级响应。' : '若状态分数或压力信号继续恶化，应升级响应。',
    executionNotes: [
      '当前已启用回退动作队列。',
...
function buildTriggerMonitorFallback(data = {}, metadata = {}, strategyState = 'Caution') {
  return {
    upgradeTriggers: [
      '若总风险分数在当前位置继续上升，则升级响应。',
      '若未来近3日短期恶化重新启动，则升级响应。',
      '若预警强度或数据降级继续增加，则升级响应。'
    ],
...
function buildHistoricalRegimeFallback(data = {}, metadata = {}, decisionState = {}) {
  ...
  return {
    ...
    interpretationNote: '该层仅用于历史解释，不会改变当前策略状态。'
  };
}
...
function createDecisionFallback(data = {}, metadata = {}) {
  ...
  return {
    ...
    dominantDrivers: [{
      ...
      trend: 0,
        reason: '在决策模型恢复前，请先参考基线风险分数和现有页面模块。'
    }],
    positionGuidance: {
...
function buildDominantDrivers(data, metadata) {
  ...
  if (metadata.realtimeFallbackUsed) {
    moduleEntries.push({
      ...
      trend: 0,
      reason: '实时回退或本地模式已启用，当前应压低决策置信度。'
    });
  }
...
function buildTriggerMonitorEngine(data, metadata, decisionState, positionGuidance, actionQueue, dominantDrivers) {
  try {
    ...
    const upgradeTriggers = uniqTexts([
      '若状态分数较当前阶段再上升 8 分或以上，则升级。',
      '若近3日变化转正且超过 +6，则升级。',
      '若 70 以上模块共振扩大到 4 个或以上，则升级。',
      '若 80 以上严重共振扩大到 3 个或以上，则升级。',
      '若红色告警增加或出现新的关键告警，则升级。',
      '若流动性、波动或资金压力再度加速，则升级。',
      metadata.realtimeCacheOnly ? '若仅缓存模式持续到下一周期，则升级。' : '',
      metadata.realtimeFreshnessLevel === 'stale' || metadata.realtimeUnavailable ? '若陈旧数据或仅基线模式持续且未恢复，则升级。' : ''
    ]).slice(0, 7);
...
function buildInvalidationRulesEngine(data, metadata, decisionState, positionGuidance, actionQueue, dominantDrivers) {
  try {
    ...
    const invalidationSignals = uniqTexts([
      '若总风险分数在当前位置再上升 8 分，则当前防守判断失效。',
      '若近3日趋势停止缓和并重新回到 +3 以上，则当前判断失效。',
      '若共振广度再次扩大，则当前判断失效。',
      '若红色告警增加，则当前判断失效。',
      metadata.realtimeCacheOnly ? '若仅缓存模式持续且压力信号维持高位，则当前判断失效。' : '',
      metadata.realtimeUnavailable ? '若仅基线模式持续且告警恶化，则当前判断失效。' : ''
    ]).slice(0, 6);
  ...
  try {
    ...
    const candidates = [
      ...
      evaluateCandidate({
        ...
        evaluate: ({ addMatch, addMismatch }) => {
          ...
          addMatch(energy >= t.energy.min, t.energy.points, `能源压力升至 ${energy}，仍处高位。`);
      ...
      evaluateCandidate({
        ...
        evaluate: ({ addMatch, addMismatch }) => {
          ...
          addMatch(geopolitical >= t.regionalDrivers.geopoliticalMin || energy >= t.regionalDrivers.energyMin, t.regionalDrivers.points, `地缘与能源驱动仍占主导，当前地缘 ${geopolitical}、能源 ${energy}。`);
      ...
      evaluateCandidate({
        ...
        evaluate: ({ addMatch, addMismatch }) => {
          ...
          addMatch(stateScore >= t.stateScore.min && stateScore <= t.stateScore.max, t.stateScore.points, `状态分数 ${stateScore} 处于高位但未进入危机区间。`);
    ...
    return {
      ...
      secondaryMatch: secondary
        ...
        : null,
      confidence,
      interpretationNote: '该历史相似层仅用于解释，不会改变当前策略状态、仓位建议或动作队列。'
    };
  } catch (error) {
...
function buildDecisionModel(data, history, metadata, healthDashboard) {
  try {
    ...
    const driverLabels = dominantDrivers.map((item) => item.label).join('、') || '基础驱动项';
    ...
    const decisionModel = {
      ...
      positionGuidance: {
        ...
        ...buildLegacyPositionGuidanceCompat(positionGuidanceWithPermissions, position, [
          executionLock.description || '按当前执行锁状态运行。',
          `健康状态：${formatHealthLevelLabel(healthDashboard.overallLevel)}。`,
      ...
      actionQueue: {
        ...
        ...buildLegacyActionQueueCompat(actionQueueWithUpgrade, [
            executionLock.description || '按当前执行锁状态运行。',
            actionLayer.todayAction || '请以当前动作队列作为主要执行依据。'
        ])
      },
      triggerMonitor,
      invalidationRules,
...
function applyRealtimeOverlay(base, realtimePayload) {
  ...
  next.tradingSystem.riskControl.resetThresholds = [
    '波动率指数 VIX < 18 且高收益利差 HY OAS < 3.7：才允许回到绿色状态。',
    '布伦特原油 < 95 且美元指数 DXY 走弱：才允许提高成长仓。',
  ...
  next.tradingSystem.signalEngine = {
    ...
    notes: [
      `执行引擎状态：${levelLabel}。`,
      `关键快变量：布伦特原油 ${fmtNumSafe(brent,1)} / 美元指数 ${fmtNumSafe(dxy,2)} / 波动率指数 VIX ${fmtNumSafe(vix,2)} / 高收益利差 ${fmtNumSafe(hy,2)}。`,
  ...
  next.decisionLine = `当前已进入 v26.0B-pr6 交易引擎模式：实时快变量 ${realtimePayload.sourceMode === 'live' ? '实时模式' : realtimePayload.sourceMode === 'cache' ? '缓存模式' : realtimePayload.sourceMode || '--'}，执行状态灯为 ${levelLabel}。先看状态灯，再决定能不能动。`;
  next.summary = `v26.0B-pr6 正根据混合实时架构输出交易引擎结论。最新快变量：布伦特原油 ${fmtNumSafe(brent,1)}、美元指数 ${fmtNumSafe(dxy,2)}、波动率指数 VIX ${fmtNumSafe(vix,2)}、高收益利差 HY OAS ${fmtNumSafe(hy,2)}%。`;
  next.recovery = {
    degradedMode: !!realtimePayload.degradedMode,
    safeOutput: true,
    lastRun: realtimePayload.updatedAt || next.updatedAt,
    notes: realtimePayload.notes || ['v26.0B-pr6 快变量正常。']
  };

  next.decisionLine = String(next.decisionLine || '').replace(/v26\.0A-rc1/g, 'v26.0B-pr6');
  next.summary = String(next.summary || '').replace(/v26\.0A-rc1/g, 'v26.0B-pr6');
  if (Array.isArray(next.recovery?.notes)) {
    next.recovery.notes = next.recovery.notes.map((note) => String(note).replace(/v26\.0A-rc1/g, 'v26.0B-pr6'));
  }
  return next;
}
...
function describeStateChange(stateMeta = {}) {
  ...
  const highRiskStreakDays = Number(stateMeta.highRiskStreakDays) || 0;
    ...
    if (delta >= 8) return '近 3 日持续上行';
    if (delta >= 3) return '近 3 日偏强走高';
    if (delta <= -8) return extremeCount > 0 || highRiskStreakDays >= 3 ? '高位仍在，但已开始缓和' : '近 3 日明显回落';
    if (delta <= -3) return '正在趋稳';
    if (extremeCount > 0) return '仍维持在高位区间';
    return '接近当前状态中枢';
  }
...
function buildDecisionHeaderModel(decisionModel = {}, data = {}) {
  ...
  return {
    ...
    stateChange: describeStateChange(decisionModel.stateMeta || {}),
      ...
      reason: decisionModel.stateReason || data?.decisionLine || '当前结论由 v26 决策模型统一汇总生成。',
      escalationLabel: decisionModel?.triggerMonitor?.escalationLevel
        ? `升级监控：${({ low: '低', medium: '中', high: '高', severe: '极高' })[decisionModel.triggerMonitor.escalationLevel] || decisionModel.triggerMonitor.escalationLevel}`
        : '升级监控已开启',
      cashGuidance: decisionModel?.positionGuidance?.cashGuidance || '保持基线现金纪律。',
      newExposurePolicy: decisionModel?.positionGuidance?.newExposurePolicy || '新增风险暴露仅允许分步执行。',
...
function renderDecisionHeader(model) {
  ...
  drivers.innerHTML = '';
    (model.dominantRiskSources || ['风险来源暂不可用']).forEach((driver) => {
