...
- `npm run check:all` 当前由 `package.json` 定义，包含 74 个串联检查项。
...
- 当前 M-74 摘要：shipping/freight (BDTI/BCTI/BDI)、Fed target/SEP/FOMC text/ZQ futures proxy、MRTS 细分消费、BIZD private-credit public proxy、VNQ/REM CRE public proxy 均进入 audit-only/display-only 生产数据与前端展示；OIS/CDX/private marks/non-public CRE 保持 `manual_required`；`check:all` = 74。
