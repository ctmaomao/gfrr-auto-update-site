...
当前处于 `v28.0J` 稳定观察基线；页面公开标签仍为 `v28.0C`，不要把工程内部版本同步误改成 UI 公开版本。v28.0J-2B post-deploy audit 已通过，当前前端版本为 `28.0M-80V`。
...
- Frontend asset cache version 当前为 `28.0M-80V`；修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js` 时必须同步 bump。
...
- Public Source Tranche 6 M-80: adds audit-only/display-only CheckMySwap USD OIS public curve (`macroDrivers.policyExpectations.oisForwardCurve`), Yahoo CMBS ETF public proxy inside `macroDrivers.commercialRealEstate`, and Yahoo PBDC/SRLN listed private-credit / senior-loan proxies inside `macroDrivers.privateCreditProxy`. CDX HY/IG, private marks, Platts Dated Brent, official ICE settlement curve, Redbook raw subscription feed, BoA raw card feed, and non-public CRE loan tape remain unconnected/manual; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-80V.
...
- 页面公开标签：`v28.0C`。
- 工程稳定观察基线：`v28.0J`。
- 当前 frontend asset cache version：`28.0M-80V`。
