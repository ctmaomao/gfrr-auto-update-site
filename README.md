...
当前处于 `v28.0J` 稳定观察基线；页面公开标签仍为 `v28.0C`，不要把工程内部版本同步误改成 UI 公开版本。v28.0J-2B post-deploy audit 已通过，当前前端版本为 `28.0M-84V`。
...
- Frontend asset cache version 当前为 `28.0M-84V`；修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js` 时必须同步 bump。
...
- 页面公开标签：`v28.0C`。
- 工程稳定观察基线：`v28.0J`。
- 当前 frontend asset cache version：`28.0M-84V`。
