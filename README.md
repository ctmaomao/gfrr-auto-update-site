...
当前处于 `v28.0J` 稳定观察基线；页面公开标签仍为 `v28.0C`，不要把工程内部版本同步误改成 UI 公开版本。v28.0J-2B post-deploy audit 已通过，当前前端版本为 `28.0M-70V`。
...
- Frontend asset cache version 当前为 `28.0M-70V`；修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js` 时必须同步 bump。
- `npm run check:all` 当前由 `package.json` 定义，包含 46 个串联检查项；历史视觉与 Market Pricing 回归守护通过 suite 聚合保留覆盖。
...
- Check Suite Slimming M-73: consolidates historical frontend visual checks and Market Pricing / cross-validation regression checks into two suite entries while keeping every original `check:*` script runnable on its own. `check:all` shrinks from 73 to 46 top-level items without removing guard coverage. No data, frontend runtime, workflow, scoring, decision, execution, or position change.
