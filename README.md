...
当前处于 `v28.0J` 稳定观察基线；页面公开标签仍为 `v28.0C`，不要把工程内部版本同步误改成 UI 公开版本。v28.0J-2B post-deploy audit 已通过，当前前端版本为 `28.0M-86V`。
...
- Frontend asset cache version 当前为 `28.0M-86V`；修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js` 时必须同步 bump。
- `npm run check:all` 当前由 `package.json` 定义，包含 23 个顶层 suite / 检查项；suite 内仍运行原子检查脚本。
...
- ISM Manufacturing PMI M-47/M-67: M-47 added the consumer PMI fields and narrative branches; M-67 repairs the source by parsing ISM's official public Manufacturing PMI report page. `macroDrivers.consumer.source` is now `FRED:UMCSENT; ISM:ManufacturingPMI`; PMI remains audit-only/display-only and never enters scoring or decision logic.
- Employment breadth M-68: adds `macroDrivers.employment` with FRED:ICSA + FRED:CCSA weekly claims and FRED:JTSJOL monthly openings. The layer is audit-only/display-only, feeds only the Macro Drivers runtime card, stays out of scoring/decision/execution/position, and adds `check:macro-drivers-employment` (`check:all` = 70). Cache bumped to 28.0M-68V.
- Consumer Retail M-69: adds `macroDrivers.consumerRetail` with FRED:CARTS + FRED:CARTSR Chicago Fed weekly retail nowcast. The layer is audit-only/display-only, feeds only the Macro Drivers runtime card, stays out of scoring/decision/execution/position, does not touch displayInputsBaseline/effectiveDisplayInputs/cross-validation, and adds `check:macro-drivers-consumer-retail` (`check:all` = 71). Cache bumped to 28.0M-69V; CARTSP is not ingested.
- Commercial Real Estate M-70: adds `macroDrivers.commercialRealEstate` with FRED:DRCRELEXFACBS + FRED:CORCREXFACBS + FRED:SUBLPDRCSN/SUBLPDRCSC/SUBLPDRCSM quarterly CRE credit pressure data. The layer is audit-only/display-only, feeds only the Macro Drivers runtime card, stays out of scoring/decision/execution/position, does not extend `macroDrivers.credit`, and adds `check:macro-drivers-commercial-real-estate` (`check:all` = 72). Cache bumped to 28.0M-70V; CDX/private credit are P3 source-review candidates.
- Brent Public Proxy Source Review M-71: identifies EIA Europe Brent Spot Price FOB, ICE Brent futures curve, Baltic Exchange freight benchmarks, Freightos Baltic Index, and future licensed S&P/Platts Dated Brent as candidate source families only. Adds `check:brent-public-proxy-source-review` (`check:all` = 73). No live fetch, source approval, production write, frontend, workflow, Worker runtime, Brent promotion, scoring, decision, execution, or position change.
- Macro Data Auto Ingestion M-73: repairs BGCR/TGCR repo-market spread production fetch by switching from invalid FRED ids to the official NY Fed secured rates API, and extends `macroDrivers.employment` with FRED average hourly earnings, U-6, and a public payroll-industry diffusion proxy. The layer remains audit-only/display-only, feeds only the existing Macro Drivers UI, stays out of scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, and cross-validation; `check:all` remains 73. Cache bumped to 28.0M-73V.
- Expanded Macro Data Auto Ingestion M-74: adds audit-only/display-only `macroDrivers.shippingFreight` (StockQ BDTI/BCTI/BDI), `macroDrivers.policyExpectations` (FRED target range/DFF + Yahoo ZQ=F + Federal Reserve SEP/FOMC statement), `macroDrivers.privateCreditProxy` (Yahoo BIZD + FRED HY OAS), FRED MRTS retail segment diffusion, and VNQ/REM CRE public market proxies. OIS/CDX/private marks/non-public CRE remain `manual_required`; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes; `check:all` = 74. Cache bumped to 28.0M-74V.
- Check Suite Compaction M-75: adds `scripts/check-suite.mjs` and groups same-family checks into top-level suites (`external-ai`, `market-pricing`, `macro-drivers`, `brent`, `narrative-density`, `world-order-acled`, `frontend-visual-history`). `check:all` top-level entries reduce 74 → 23 while preserving all atomic checks. No runtime, frontend, workflow, scoring, decision, execution, position, or data change; no cache bump.
- Frontend Macro Field Surfacing M-76: expands Macro Drivers / Risk Engines / Brent detail evidence to consume already-present backend fields: policy target range, ZQ front price, SEP full medians, FOMC tone counts, BGCR/TGCR raw rates and spreads, all MRTS retail segments, BCTI/BDI daily changes, CDX/private-mark manual statuses, VNQ/REM timestamps, employment source statuses, NFCI/SLOOS/IG deltas, and ULSD/crack-spread 4w fields. Frontend display only; no data, workflow, scoring, decision, execution, position, or cross-validation behavior change. Cache bumped to 28.0M-76V.
- Public Source Tranche 3 M-77: adds audit-only/display-only Federal Reserve FOMC minutes keyword NLP tone/topic counts, BoA Consumer Checkpoint public HTML card-spending YoY summary, and ICE Brent futures contract structure-only (`brentPricingLayer.futuresCurve`). ICE does not supply priced settlement curve here, BoA is not Redbook/raw card feed, and OIS/CDX/private marks/non-public CRE remain unconnected/manual; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-77V.
- Public Source Tranche 4 M-78: adds audit-only/display-only Yahoo ZQ monthly Fed funds futures proxy curve, FRED IG OAS (`BAMLC0A0CM`) inside `macroDrivers.privateCreditProxy`, and Yahoo BZ monthly Brent priced futures proxy (`brentPricingLayer.futuresPriceCurve`). OIS forward, CDX HY/IG, private marks, Platts Dated Brent, official ICE settlement curve, and non-public CRE remain unconnected/manual; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-78V.
- Public Source Tranche 5 M-79: adds audit-only/display-only Trading Economics Redbook public HTML same-store sales YoY summary in `macroDrivers.consumerRetail`, plus Yahoo SR3 monthly Three-Month SOFR futures proxy curve in `macroDrivers.policyExpectations.sofrFuturesCurve`. OIS forward remains `manual_required`; Redbook raw subscription feed / BoA raw card feed remain unconnected; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-79V.
- Public Source Tranche 6 M-80: adds audit-only/display-only CheckMySwap USD OIS public curve (`macroDrivers.policyExpectations.oisForwardCurve`), Yahoo CMBS ETF public proxy inside `macroDrivers.commercialRealEstate`, and Yahoo PBDC/SRLN listed private-credit / senior-loan proxies inside `macroDrivers.privateCreditProxy`. At M-80 close, CDX HY/IG, private marks, Platts Dated Brent, official ICE settlement curve, Redbook raw subscription feed, BoA raw card feed, and non-public CRE loan tape remained unconnected/manual; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-80V.
- Public Source Tranche 7 M-81: adds audit-only/display-only ICE public CDX index settlement prices (`macroDrivers.privateCreditProxy.cdxHyPrice/cdxIgPrice`) from ICE Clear Credit index instruments. Private credit marks, Platts Dated Brent, official ICE Brent settlement curve, Redbook raw subscription feed, BoA raw card feed, and non-public CRE loan tape remain unconnected/manual; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-81V.
- Public Source Tranche 8 M-82: adds audit-only/display-only ICE public delayed Brent futures last-price curve (`brentPricingLayer.iceFuturesPriceCurve`) from ICE product-guide contract-data. This is not Platts Dated Brent, not official ICE settlement curve, and not formal physical spot evidence; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-82V.
- Public Source Tranche 9 M-83: adds audit-only/display-only CCLFX public interval-fund NAV proxy (`macroDrivers.privateCreditProxy.intervalFundNavPrice`) from Yahoo chart data for Cliffwater Corporate Lending Fund. This is not private credit marks, not fundraising data, and not a licensed private-loan valuation feed; `privateCreditMarksStatus` remains `manual_required`; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-83V.
- Public Source Tranche 10 M-84: adds audit-only/display-only FRED `CREACBW027SBOG` public weekly aggregate bank CRE loan balance / exposure stock proxy (`macroDrivers.commercialRealEstate.creLoanBalance`). This is not non-public CRE loan tape, not private CRE marks, not loan-level exposure data, and not CDX/private-credit data; `nonPublicCreStatus` remains `manual_required`; no scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-84V.
- Public Source Tranche 11 M-85: adds audit-only/display-only EIA Europe Brent Spot Price FOB public HTML proxy (`brentPricingLayer.eiaBrentSpotProxy`). This is not Platts Dated Brent, not formal Dated Brent, and not physical transaction evidence; no `values.brent`, Brent promotion, scoring/decision/execution/position, worker/realtime, displayInputsBaseline/effectiveDisplayInputs, or cross-validation changes. Cache bumped to 28.0M-85V.
- Frontend Proxy Coverage Semantics M-86: Macro Overview now separates live public proxy coverage from formal/non-public source boundary notes. `missingEvidence` is reserved for actual unavailable public data; Platts/official settlement/private marks/non-public tape remain boundary notes. Frontend display only; no data, scoring, decision, execution, position, worker, workflow, or cross-validation changes. Cache bumped to 28.0M-86V.
...
- Repo Market Spread M-50/M-73: Adds BGCR/TGCR NY Fed reference rates to resolveFedLiquidity and M-73 switches runtime fetch to the official NY Fed secured rates API after the original FRED ids proved unavailable. Extends macroDrivers.fedLiquidity with bgcr/tgcr spread fields and keeps storage as %, display as bp (×100).
...
- M-56 validate-data consumer source whitelist: accepts legacy `FRED:UMCSENT` snapshots and later multi-source consumer labels, unblocking Build #74 daily refresh and activating M-46~M-50 refreshed fields in `data/radar-data.json`. Backend validator fix only; no frontend cache bump.
...
- M-67 ISM PMI source repair: replaces the broken FRED PMI path with a low-frequency parser for ISM's public Manufacturing PMI report page. PMI remains audit-only/display-only; `check:all` stays 69. No frontend cache bump.
- M-68 employment breadth: adds `macroDrivers.employment` from FRED ICSA/CCSA/JTSJOL with per-series fallback status, frontend `driver-employment` surfacing, and `check:macro-drivers-employment` (`check:all` = 70). Cache bumped to 28.0M-68V. No scoring/decision/execution/position, worker, realtime, or cross-validation changes.
- M-69 consumerRetail: adds `macroDrivers.consumerRetail` from FRED CARTS/CARTSR with per-series fallback status, frontend `driver-consumer-retail` surfacing, and `check:macro-drivers-consumer-retail` (`check:all` = 71). Cache bumped to 28.0M-69V. Redbook/BoA Card are P3 source-review candidates; no scoring/decision/execution/position, worker, realtime, or cross-validation changes.
- M-70 commercialRealEstate: adds `macroDrivers.commercialRealEstate` from FRED DRCRELEXFACBS/CORCREXFACBS/SUBLPDRCSN/SUBLPDRCSC/SUBLPDRCSM with per-series fallback status, frontend `driver-cre` surfacing, and `check:macro-drivers-commercial-real-estate` (`check:all` = 72). Cache bumped to 28.0M-70V. CDX/private credit are P3 source-review candidates; no scoring/decision/execution/position, worker, realtime, macroDrivers.credit, or cross-validation changes.
