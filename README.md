...
当前处于 `v28.0J` 稳定观察基线；页面公开标签仍为 `v28.0C`，不要把工程内部版本同步误改成 UI 公开版本。v28.0J-2B post-deploy audit 已通过，当前前端版本为 `28.0M-75V`。
...
- Frontend asset cache version 当前为 `28.0M-75V`；修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js` 时必须同步 bump。
- `npm run check:all` 当前由 `package.json` 定义，包含 75 个串联检查项。
...
- Macro Driver Date Display Fix M-72: fixes JOLTS month and CRE quarter vintage rendering so malformed period labels no longer appear on Macro Drivers cards. Cache bumped to 28.0M-72V; no data, workflow, Worker, scoring, decision, execution, or position change.
- Brent Physical Proof-of-Source Design M-74: starts the three Brent physical/term/freight tracks by defining licensed Platts Dated Brent, ICE Brent futures curve, Baltic freight, and Freightos FBX proof contracts. Adds `check:brent-physical-proof-of-source-design` (`check:all` = 74). No live fetch, production write, frontend, workflow, Worker runtime, Brent promotion, scoring, decision, execution, or position change.
- Brent Production Proxy Display M-75: adds production Daily `brentPricingLayer.termStructureProxy` from Yahoo delayed Brent futures contract chart payloads (`BZ*.NYM`) with short timeout/failure isolation, surfaces `formalDatedBrent`, `termStructureProxy`, and `shippingFreightProxy` in the existing Brent frontend card, and adds `check:brent-production-proxy-display` (`check:all` = 75). Formal Platts Dated Brent and freight values remain null/status-only until licensed or stable sources are approved. Cache bumped to 28.0M-75V; no workflow, Worker runtime, Brent promotion, `values.brent`, scoring, decision, execution, or position change.
