...
当前处于 `v28.0J` 稳定观察基线；页面公开标签仍为 `v28.0C`，不要把工程内部版本同步误改成 UI 公开版本。v28.0J-2B post-deploy audit 已通过，当前前端版本为 `28.0M-73V`。
...
- Frontend asset cache version 当前为 `28.0M-73V`；修改 `index.html`、`scripts/app.js` 或 `scripts/modules/*.js` 时必须同步 bump。
...
- Macro Driver Date Display Fix M-72: fixes JOLTS month and CRE quarter vintage rendering so ISO date strings no longer surface malformed period labels. Cache bumped to 28.0M-72V; no data, workflow, Worker, scoring, decision, execution, or position change.
- Null-zero Display Guard M-73: fixes missing repo spread / ICE Brent diagnostic values so `null` cannot render or classify as `+0.0bp` / `0.00`. Adds `check:null-zero-display-guards` (`check:all` = 74). Cache bumped to 28.0M-73V; no data, workflow, Worker, scoring, decision, execution, or position change.
